...
module.exports = function(err, req, res) {
  ...
  case msg.includes('Cannot move that direction'): return res.status(406).send(`${ err.name }: ${ err.message }`);
  ...
  case msg.includes('Cannot move in that direction'): return res.status(400).send(`${ err.name }: ${ err.message }`);
